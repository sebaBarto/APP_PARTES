...
function armarClausulas(d) {
  return [
    ...
    { tipo: "parrafo-negrita", texto: `Abono mensual: $ ${d.abono_mensual} + IVA x mes` },
...
async function generarComodatoPDF(datos, firmaBase64) {
  ...
  // Las imágenes se ajustan dentro de un cuadro máximo (ancho Y alto)
  // en vez de fijar solo el ancho — la firma de Thiesing en particular
  // es casi cuadrada (315x287px), así que fijar solo el ancho la hacía
  // demasiado alta y terminaba tapando el párrafo de arriba.
  function ajustarDentroDe(img, maxAncho, maxAlto) {
    const escala = Math.min(maxAncho / img.width, maxAlto / img.height);
    return { width: img.width * escala, height: img.height * escala };
  }

  nuevaPaginaSiHaceFalta(150);
  y -= 40; // antes 20 — poco margen fue justo la otra causa del solape
  const colIzqX = MARGEN;
  const colDerX = MARGEN + ANCHO_TEXTO / 2 + 10;
  const yFirmas = y;
  const ALTO_MAX_FIRMA = 34;
  ...
  try {
    ...
    const dimTh = ajustarDentroDe(imgFirmaThiesing, 90, ALTO_MAX_FIRMA);
    pagina.drawImage(imgFirmaThiesing, { x: colIzqX + 20, y: yFirmas + 4, width: dimTh.width, height: dimTh.height });
  } catch (err) {
  ...
  if (firmaBase64) {
    try {
      ...
      const dimCliente = ajustarDentroDe(firmaPng, 110, ALTO_MAX_FIRMA);
      pagina.drawImage(firmaPng, { x: colDerX, y: yFirmas + 2, width: dimCliente.width, height: dimCliente.height });
    } catch (err) {
